...
use dust_converter::{self, config::{MAX_FEE_PERCENTAGE, MAX_PERCENTAGE}};
...
fn test_swap_dust_token_success() {
    ...
    setup.swap_dust_token(&payments, &caller_address, None);
...
fn test_swap_dust_token_pair_fail() {
    ...
    let caller_address = setup.owner.clone();
    setup.swap_dust_token(&payments, &caller_address, Some("Not enough reserve"));
...
fn test_sell_dust_tokens() {
    ...
    setup.swap_dust_token(&payments, &user, None);
